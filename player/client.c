/* Copyright (C) 2017 the dmpv developers
 *
 ...
 * Locking hierarchy:
 *
 *  MPContext > mp_client_api.lock > dmpv_handle.lock > * > dmpv_handle.wakeup_lock
 *
...
struct mp_client_api {
    ...
    struct dmpv_render_context *render_context;
};

struct observe_property {
    // -- immutable
    struct dmpv_handle *owner;
    char *name;
...
struct dmpv_handle {
    ...
    // -- not thread-safe
    struct dmpv_event *cur_event;
    struct dmpv_event_property cur_property_event;
    ...
    dmpv_event *events;      // ringbuffer of max_events entries
    ...
    // is used to safely unlock dmpv_handle.lock while reading a property. If
...
static bool gen_log_message_event(struct dmpv_handle *ctx);
static bool gen_property_change_event(struct dmpv_handle *ctx);
static void notify_property_events(struct dmpv_handle *ctx, int event);
...
void mp_clients_destroy(struct MPContext *mpctx)
{
    ...
    // The API user is supposed to call dmpv_render_context_free(). It's simply
    // not allowed not to do this.
    if (mpctx->clients->render_context) {
        MP_FATAL(mpctx, "Broken API use: dmpv_render_context_free() not called.\n");
        abort();
    }
...
// at least called dmpv_wait_event() at least once since creation (or exited).
...
static struct dmpv_handle *find_client_id(struct mp_client_api *clients, int64_t id)
{
...
static struct dmpv_handle *find_client(struct mp_client_api *clients,
                                      const char *name)
{
    if (name[0] == '@') {
        char *end;
...
struct dmpv_handle *mp_new_client(struct mp_client_api *clients, const char *name)
{
    ...
    struct dmpv_handle *client = talloc_ptrtype(NULL, client);
    *client = (struct dmpv_handle){
        .log = mp_log_new(client, clients->mpctx->log, nname),
        .mpctx = clients->mpctx,
        .clients = clients,
        .id = ++(clients->id_alloc),
        .cur_event = talloc_zero(client, struct dmpv_event),
        .events = talloc_array(client, dmpv_event, num_events),
        .max_events = num_events,
        .event_mask = 1ULL << DMPV_EVENT_SHUTDOWN,  // shutdown is always sent, others should be requested
        .wakeup_pipe = {-1, -1},
    };

    const char *me = getenv("DMPV_EVENTS_ALL");  // "1" enables all except tick
    if (me && !strcmp("1", me))
        client->event_mask = ((1ULL << INTERNAL_EVENT_BASE) - 1) & ~(1ULL << DMPV_EVENT_TICK);
...
int64_t dmpv_client_id(dmpv_handle *ctx)
{
    return ctx->id;
}
...
static void unlock_core(dmpv_handle *ctx)
{
...
static void mp_destroy_client(dmpv_handle *ctx, bool terminate)
{
    ...
    if (mpctx->is_cli) {
        ...
        // If the last strong dmpv_handle got destroyed, destroy the core.
...
void dmpv_destroy(dmpv_handle *ctx)
{
    mp_destroy_client(ctx, false);
}

void dmpv_terminate_destroy(dmpv_handle *ctx)
{
...
dmpv_handle *dmpv_create(void)
{
    ...
    m_config_set_profile(mpctx->mconfig, "dmpv", 0);

    dmpv_handle *ctx = mp_new_client(mpctx->clients, "main");
    if (!ctx) {
        mp_destroy(mpctx);
        return NULL;
    }
...
dmpv_handle *dmpv_create_client(dmpv_handle *ctx, const char *name)
{
    if (!ctx)
        return dmpv_create();
    dmpv_handle *new = mp_new_client(ctx->mpctx->clients, name);
    if (new)
        dmpv_wait_event(new, 0); // set fuzzy_initialized
    return new;
}

dmpv_handle *dmpv_create_weak_client(dmpv_handle *ctx, const char *name)
{
    dmpv_handle *new = dmpv_create_client(ctx, name);
    if (new)
        mp_client_set_weak(new);
    return new;
}

int dmpv_initialize(dmpv_handle *ctx)
{
    lock_core(ctx);
    int res = mp_initialize(ctx->mpctx, NULL) ? DMPV_ERROR_INVALID_PARAMETER : 0;
...
static void dup_event_data(struct dmpv_event *ev)
{
    switch (ev->event_id) {
    case DMPV_EVENT_CLIENT_MESSAGE: {
        struct dmpv_event_client_message *src = ev->data;
        struct dmpv_event_client_message *msg =
            talloc_zero(NULL, struct dmpv_event_client_message);
    ...
    case DMPV_EVENT_START_FILE:
        ev->data = talloc_memdup(NULL, ev->data, sizeof(dmpv_event_start_file));
        break;
    case DMPV_EVENT_END_FILE:
        ev->data = talloc_memdup(NULL, ev->data, sizeof(dmpv_event_end_file));
        break;
    default:
...
static int append_event(struct dmpv_handle *ctx, struct dmpv_event event, bool copy)
{
    ...
    if (event.event_id == DMPV_EVENT_SHUTDOWN)
        ctx->event_mask &= ctx->event_mask & ~(1ULL << DMPV_EVENT_SHUTDOWN);
    return 0;
}
...
static void send_reply(struct dmpv_handle *ctx, uint64_t userdata,
...
static const bool deprecated_events[] = {
    [DMPV_EVENT_IDLE] = true,
    [DMPV_EVENT_TICK] = true,
};

int dmpv_request_event(dmpv_handle *ctx, dmpv_event_id event, int enable)
{
    if (!dmpv_event_name(event) || enable < 0 || enable > 1)
...
static void set_wait_for_hook_flags(dmpv_handle *ctx)
{
    for (int n = 0; n < ctx->num_properties; n++) {
...
static bool check_for_for_hook_flags(dmpv_handle *ctx)
{
    for (int n = 0; n < ctx->num_properties; n++) {
...
dmpv_event *dmpv_wait_event(dmpv_handle *ctx, double timeout)
{
    ...
    *event = (dmpv_event){0};
    ...
    while (1) {
        ...
        if (ctx->choked && !ctx->num_events) {
            ctx->choked = false;
            event->event_id = DMPV_EVENT_QUEUE_OVERFLOW;
            break;
        }
        struct dmpv_event *ev =
            ctx->num_events ? &ctx->events[ctx->first_event] : NULL;
        if (ev && ev->event_id == DMPV_EVENT_HOOK) {
...
static const struct m_option type_conv[] = {
    [DMPV_FORMAT_STRING]     = { .type = CONF_TYPE_STRING },
    [DMPV_FORMAT_FLAG]       = { .type = CONF_TYPE_FLAG },
    [DMPV_FORMAT_INT64]      = { .type = CONF_TYPE_INT64 },
    [DMPV_FORMAT_DOUBLE]     = { .type = CONF_TYPE_DOUBLE },
    [DMPV_FORMAT_NODE]       = { .type = CONF_TYPE_NODE },
};

static const struct m_option *get_mp_type(dmpv_format format)
{
...
// for read requests - DMPV_FORMAT_OSD_STRING special handling
static const struct m_option *get_mp_type_get(dmpv_format format)
{
    if (format == DMPV_FORMAT_OSD_STRING)
        format = DMPV_FORMAT_STRING; // it's string data, just other semantics
...
static bool conv_node_to_format(void *dst, dmpv_format dst_fmt, dmpv_node *src)
{
    ...
    if (dst_fmt == DMPV_FORMAT_DOUBLE && src->format == DMPV_FORMAT_INT64) {
        *(double *)dst = src->u.int64;
        return true;
    }
    if (dst_fmt == DMPV_FORMAT_INT64 && src->format == DMPV_FORMAT_DOUBLE) {
...
void dmpv_free_node_contents(dmpv_node *node)
{
    static const struct m_option type = { .type = CONF_TYPE_NODE };
    m_option_free(&type, node);
}

int dmpv_set_option(dmpv_handle *ctx, const char *name, dmpv_format format,
                   void *data)
{
    const struct m_option *type = get_mp_type(format);
    if (!type)
        return DMPV_ERROR_OPTION_FORMAT;
    struct dmpv_node tmp;
    if (format != DMPV_FORMAT_NODE) {
        tmp.format = format;
    ...
    case M_OPT_INVALID:
        return DMPV_ERROR_OPTION_ERROR;
    case M_OPT_OUT_OF_RANGE:
        return DMPV_ERROR_OPTION_FORMAT;
    case M_OPT_UNKNOWN:
        return DMPV_ERROR_OPTION_NOT_FOUND;
    default:
        if (err >= 0)
            return 0;
        return DMPV_ERROR_OPTION_ERROR;
...
int dmpv_set_option_string(dmpv_handle *ctx, const char *name, const char *data)
{
    return dmpv_set_option(ctx, name, DMPV_FORMAT_STRING, &data);
}

// Run a command in the playback thread.
static void run_locked(dmpv_handle *ctx, void (*fn)(void *fn_data), void *fn_data)
{
    mp_dispatch_lock(ctx->mpctx->dispatch);
    fn(fn_data);
    mp_dispatch_unlock(ctx->mpctx->dispatch);
...
static int run_async(dmpv_handle *ctx, void (*fn)(void *fn_data), void *fn_data)
{
...
static void cmd_complete(struct mp_cmd_ctx *cmd)
{
    ...
    req->status = cmd->success ? 0 : DMPV_ERROR_COMMAND;
    if (req->res) {
        *req->res = cmd->result;
        cmd->result = (dmpv_node){0};
    }
...
static int run_client_command(dmpv_handle *ctx, struct mp_cmd *cmd, dmpv_node *res)
{
    if (!cmd)
        return DMPV_ERROR_INVALID_PARAMETER;
    if (!ctx->mpctx->initialized) {
        talloc_free(cmd);
        return DMPV_ERROR_UNINITIALIZED;
    }
...
int dmpv_command(dmpv_handle *ctx, const char **args)
{
    return run_client_command(ctx, mp_input_parse_cmd_strv(ctx->log, args), NULL);
}

int dmpv_command_node(dmpv_handle *ctx, dmpv_node *args, dmpv_node *result)
{
    struct dmpv_node rn = {.format = DMPV_FORMAT_NONE};
...
int dmpv_command_ret(dmpv_handle *ctx, const char **args, dmpv_node *result)
{
    struct dmpv_node rn = {.format = DMPV_FORMAT_NONE};
...
int dmpv_command_string(dmpv_handle *ctx, const char *args)
{
...
static void async_cmd_complete(struct mp_cmd_ctx *cmd)
{
    ...
    struct dmpv_event_command *data = talloc_zero(NULL, struct dmpv_event_command);
    data->result = cmd->result;
    cmd->result = (dmpv_node){0};
    talloc_steal(data, node_get_alloc(&data->result));

    struct dmpv_event reply = {
        .event_id = DMPV_EVENT_COMMAND_REPLY,
        .data = data,
        .error = cmd->success ? 0 : DMPV_ERROR_COMMAND,
    };
...
static void async_cmd_fn(void *data)
{
    ...
    if (cmd->def->can_abort) {
        ...
        abort->client_work_type = DMPV_EVENT_COMMAND_REPLY;
...
static int run_async_cmd(dmpv_handle *ctx, uint64_t ud, struct mp_cmd *cmd)
{
    if (!cmd)
        return DMPV_ERROR_INVALID_PARAMETER;
    if (!ctx->mpctx->initialized) {
...
int dmpv_command_async(dmpv_handle *ctx, uint64_t ud, const char **args)
{
    return run_async_cmd(ctx, ud, mp_input_parse_cmd_strv(ctx->log, args));
}

int dmpv_command_node_async(dmpv_handle *ctx, uint64_t ud, dmpv_node *args)
{
    return run_async_cmd(ctx, ud, mp_input_parse_cmd_node(ctx->log, args));
}

void dmpv_abort_async_command(dmpv_handle *ctx, uint64_t reply_userdata)
{
    abort_async(ctx->mpctx, ctx, DMPV_EVENT_COMMAND_REPLY, reply_userdata);
}
...
static int translate_property_error(int errc)
{
    ...
    case M_PROPERTY_ERROR:              return DMPV_ERROR_PROPERTY_ERROR;
    case M_PROPERTY_UNAVAILABLE:        return DMPV_ERROR_PROPERTY_UNAVAILABLE;
    case M_PROPERTY_NOT_IMPLEMENTED:    return DMPV_ERROR_PROPERTY_ERROR;
    case M_PROPERTY_UNKNOWN:            return DMPV_ERROR_PROPERTY_NOT_FOUND;
    case M_PROPERTY_INVALID_FORMAT:     return DMPV_ERROR_PROPERTY_FORMAT;
    // shouldn't happen
    default:                            return DMPV_ERROR_PROPERTY_ERROR;
    }
}
...
static void setproperty_fn(void *arg)
{
    ...
    struct dmpv_node tmp;
    if (req->format == DMPV_FORMAT_NODE) {
        node = req->data;
    } else {
    ...
    if (req->reply_ctx) {
        struct dmpv_event reply = {
            .event_id = DMPV_EVENT_SET_PROPERTY_REPLY,
            .error = req->status,
        };
        send_reply(req->reply_ctx, req->userdata, &reply);
        talloc_free(req);
...
int dmpv_set_property(dmpv_handle *ctx, const char *name, dmpv_format format,
                     void *data)
{
    if (!ctx->mpctx->initialized) {
        int r = dmpv_set_option(ctx, name, format, data);
        if (r == DMPV_ERROR_OPTION_NOT_FOUND &&
            mp_get_property_id(ctx->mpctx, name) >= 0)
            return DMPV_ERROR_PROPERTY_UNAVAILABLE;
        switch (r) {
        case DMPV_ERROR_SUCCESS:          return DMPV_ERROR_SUCCESS;
        case DMPV_ERROR_OPTION_FORMAT:    return DMPV_ERROR_PROPERTY_FORMAT;
        case DMPV_ERROR_OPTION_NOT_FOUND: return DMPV_ERROR_PROPERTY_NOT_FOUND;
        default:                         return DMPV_ERROR_PROPERTY_ERROR;
        }
    }
...
int dmpv_del_property(dmpv_handle *ctx, const char *name)
{
    const char* args[] = { "del", name, NULL };
    return dmpv_command(ctx, args);
}

int dmpv_set_property_string(dmpv_handle *ctx, const char *name, const char *data)
{
    return dmpv_set_property(ctx, name, DMPV_FORMAT_STRING, &data);
}
...
int dmpv_set_property_async(dmpv_handle *ctx, uint64_t ud, const char *name,
                           dmpv_format format, void *data)
{
    const struct m_option *type = get_mp_type(format);
    if (!ctx->mpctx->initialized)
        return DMPV_ERROR_UNINITIALIZED;
...
{
    struct dmpv_event_property *prop = ptr;
...
static void getproperty_fn(void *arg)
{
    ...
    case DMPV_FORMAT_DOUBLE: {
        ...
        if (err == M_PROPERTY_NOT_IMPLEMENTED) {
            ...
            if (err != M_PROPERTY_OK)
                break;
            node.format = DMPV_FORMAT_STRING;
            node.u.string = s;
        } else if (err <= 0)
            break;
        if (req->format == DMPV_FORMAT_NODE) {
            *(struct dmpv_node *)data = node;
        } else {
            if (!conv_node_to_format(data, req->format, &node)) {
                err = M_PROPERTY_INVALID_FORMAT;
                dmpv_free_node_contents(&node);
            }
        }
        break;
    }
    ...
    if (req->reply_ctx) {
        struct dmpv_event_property *prop = talloc_ptrtype(NULL, prop);
        *prop = (struct dmpv_event_property){
        ...
        struct dmpv_event reply = {
            .event_id = DMPV_EVENT_GET_PROPERTY_REPLY,
            .data = prop,
            .error = req->status,
        };
        send_reply(req->reply_ctx, req->userdata, &reply);
...
int dmpv_get_property(dmpv_handle *ctx, const char *name, dmpv_format format,
                     void *data)
{
    if (!ctx->mpctx->initialized)
        return DMPV_ERROR_UNINITIALIZED;
...
char *dmpv_get_property_string(dmpv_handle *ctx, const char *name)
{
    char *str = NULL;
    dmpv_get_property(ctx, name, DMPV_FORMAT_STRING, &str);
    return str;
}

char *dmpv_get_property_osd_string(dmpv_handle *ctx, const char *name)
{
    char *str = NULL;
    dmpv_get_property(ctx, name, DMPV_FORMAT_OSD_STRING, &str);
    return str;
}

int dmpv_get_property_async(dmpv_handle *ctx, uint64_t ud, const char *name,
                           dmpv_format format)
{
    if (!ctx->mpctx->initialized)
        return DMPV_ERROR_UNINITIALIZED;
...
int dmpv_observe_property(dmpv_handle *ctx, uint64_t userdata,
                         const char *name, dmpv_format format)
{
    const struct m_option *type = get_mp_type_get(format);
    if (format != DMPV_FORMAT_NONE && !type)
        return DMPV_ERROR_PROPERTY_FORMAT;
...
static void send_client_property_changes(struct dmpv_handle *ctx)
{
    ...
    for (int n = 0; n < ctx->num_properties; n++) {
        ...
        if (prop->format) {
            ...
            if (prop->value_valid && val_valid)
                changed = !equal_dmpv_value(&prop->value, &val, prop->format);
            if (prop->value_ts == 0)
...
static bool gen_property_change_event(struct dmpv_handle *ctx)
{
    ...
    while (1) {
            ...
            prop->value_ret_ts != prop->value_ts)   // other value than last time?
        {
            ...
            ctx->cur_property_event = (struct dmpv_event_property){
                .name = prop->name,
                .format = prop->value_valid ? prop->format : 0,
                .data = prop->value_valid ? &prop->value_ret : NULL,
            };
            *ctx->cur_event = (struct dmpv_event){
                .event_id = DMPV_EVENT_PROPERTY_CHANGE,
...
int dmpv_hook_add(dmpv_handle *ctx, uint64_t reply_userdata,
...
int dmpv_hook_continue(dmpv_handle *ctx, uint64_t id)
{
    lock_core(ctx);
...
int dmpv_load_config_file(dmpv_handle *ctx, const char *filename)
{
    lock_core(ctx);
...
// flush it, though.)
int dmpv_request_log_messages(dmpv_handle *ctx, const char *min_level)
{
...
static bool gen_log_message_event(struct dmpv_handle *ctx)
{
    if (ctx->messages) {
        ...
        if (msg) {
            struct dmpv_event_log_message *cmsg =
                talloc_ptrtype(ctx->cur_event, cmsg);
            talloc_steal(cmsg, msg);
            *cmsg = (struct dmpv_event_log_message){
                .prefix = msg->prefix,
                .level = mp_log_levels[msg->level],
                .log_level = mp_dmpv_log_levels[msg->level],
                .text = msg->text,
            };
            *ctx->cur_event = (struct dmpv_event){
                .event_id = DMPV_EVENT_LOG_MESSAGE,
                .data = cmsg,
            };
            return true;
        }
...
unsigned long dmpv_client_api_version(void)
{
    return DMPV_CLIENT_API_VERSION;
}

int dmpv_event_to_node(dmpv_node *dst, dmpv_event *event)
{
    *dst = (dmpv_node){0};

    node_init(dst, DMPV_FORMAT_NODE_MAP, NULL);
    node_map_add_string(dst, "event", dmpv_event_name(event->event_id));

    if (event->error < 0)
        node_map_add_string(dst, "error", dmpv_error_string(event->error));
    ...
    case DMPV_EVENT_END_FILE: {
        dmpv_event_end_file *eef = event->data;

        const char *reason;
        switch (eef->reason) {
        case DMPV_END_FILE_REASON_EOF: reason = "eof"; break;
        case DMPV_END_FILE_REASON_STOP: reason = "stop"; break;
        case DMPV_END_FILE_REASON_QUIT: reason = "quit"; break;
        case DMPV_END_FILE_REASON_ERROR: reason = "error"; break;
        case DMPV_END_FILE_REASON_REDIRECT: reason = "redirect"; break;
        default:
            reason = "unknown";
        }
        ...
        if (eef->reason == DMPV_END_FILE_REASON_ERROR)
            node_map_add_string(dst, "file_error", dmpv_error_string(eef->error));
        break;
    }

    case DMPV_EVENT_LOG_MESSAGE: {
        dmpv_event_log_message *msg = event->data;
    ...
    case DMPV_EVENT_CLIENT_MESSAGE: {
        dmpv_event_client_message *msg = event->data;

        struct dmpv_node *args = node_map_add(dst, "args", DMPV_FORMAT_NODE_ARRAY);
        for (int n = 0; n < msg->num_args; n++) {
            struct dmpv_node *sn = node_array_add(args, DMPV_FORMAT_NONE);
            sn->format = DMPV_FORMAT_STRING;
            sn->u.string = (char *)msg->args[n];
        }
        break;
    }

    case DMPV_EVENT_PROPERTY_CHANGE: {
        dmpv_event_property *prop = event->data;

        node_map_add_string(dst, "name", prop->name);

        switch (prop->format) {
        case DMPV_FORMAT_NODE:
            *node_map_add(dst, "data", DMPV_FORMAT_NONE) =
                *(struct dmpv_node *)prop->data;
            break;
        case DMPV_FORMAT_DOUBLE:
    ...
    case DMPV_EVENT_COMMAND_REPLY: {
        dmpv_event_command *cmd = event->data;

        *node_map_add(dst, "result", DMPV_FORMAT_NONE) = cmd->result;
        break;
    }

    case DMPV_EVENT_HOOK: {
        dmpv_event_hook *hook = event->data;
...
static const char *const err_table[] = {
    [-DMPV_ERROR_SUCCESS] = "success",
    [-DMPV_ERROR_EVENT_QUEUE_FULL] = "event queue full",
    [-DMPV_ERROR_NOMEM] = "memory allocation failed",
    [-DMPV_ERROR_UNINITIALIZED] = "core not uninitialized",
    [-DMPV_ERROR_INVALID_PARAMETER] = "invalid parameter",
    [-DMPV_ERROR_OPTION_NOT_FOUND] = "option not found",
    [-DMPV_ERROR_OPTION_FORMAT] = "unsupported format for accessing option",
    [-DMPV_ERROR_OPTION_ERROR] = "error setting option",
    [-DMPV_ERROR_PROPERTY_NOT_FOUND] = "property not found",
    [-DMPV_ERROR_PROPERTY_FORMAT] = "unsupported format for accessing property",
    [-DMPV_ERROR_PROPERTY_UNAVAILABLE] = "property unavailable",
    [-DMPV_ERROR_PROPERTY_ERROR] = "error accessing property",
    [-DMPV_ERROR_COMMAND] = "error running command",
    [-DMPV_ERROR_LOADING_FAILED] = "loading failed",
    [-DMPV_ERROR_AO_INIT_FAILED] = "audio output initialization failed",
    [-DMPV_ERROR_VO_INIT_FAILED] = "video output initialization failed",
    [-DMPV_ERROR_NOTHING_TO_PLAY] = "no audio or video data played",
    [-DMPV_ERROR_UNKNOWN_FORMAT] = "unrecognized file format",
    [-DMPV_ERROR_UNSUPPORTED] = "not supported",
    [-DMPV_ERROR_NOT_IMPLEMENTED] = "operation not implemented",
    [-DMPV_ERROR_GENERIC] = "something happened",
};

const char *dmpv_error_string(int error)
{
    error = -error;
    if (error < 0)
        error = 0;
...
static const char *const event_table[] = {
    [DMPV_EVENT_NONE] = "none",
    [DMPV_EVENT_SHUTDOWN] = "shutdown",
    [DMPV_EVENT_LOG_MESSAGE] = "log-message",
    [DMPV_EVENT_GET_PROPERTY_REPLY] = "get-property-reply",
    [DMPV_EVENT_SET_PROPERTY_REPLY] = "set-property-reply",
    [DMPV_EVENT_COMMAND_REPLY] = "command-reply",
    [DMPV_EVENT_START_FILE] = "start-file",
    [DMPV_EVENT_END_FILE] = "end-file",
    [DMPV_EVENT_FILE_LOADED] = "file-loaded",
    [DMPV_EVENT_IDLE] = "idle",
    [DMPV_EVENT_TICK] = "tick",
    [DMPV_EVENT_CLIENT_MESSAGE] = "client-message",
    [DMPV_EVENT_VIDEO_RECONFIG] = "video-reconfig",
    [DMPV_EVENT_AUDIO_RECONFIG] = "audio-reconfig",
    [DMPV_EVENT_SEEK] = "seek",
    [DMPV_EVENT_PLAYBACK_RESTART] = "playback-restart",
    [DMPV_EVENT_PROPERTY_CHANGE] = "property-change",
    [DMPV_EVENT_QUEUE_OVERFLOW] = "event-queue-overflow",
    [DMPV_EVENT_HOOK] = "hook",
};

const char *dmpv_event_name(dmpv_event_id event)
{
...
int dmpv_stream_cb_add_ro(dmpv_handle *ctx, const char *protocol, void *user_data,
                         dmpv_stream_cb_open_ro_fn open_fn)
{
    if (!open_fn)
        return DMPV_ERROR_INVALID_PARAMETER;
    ...
    for (int n = 0; n < clients->num_custom_protocols; n++) {
        ...
        if (strcmp(proto->protocol, protocol) == 0) {
            r = DMPV_ERROR_INVALID_PARAMETER;
            break;
        }
    }
