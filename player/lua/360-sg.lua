...
local draw_cropper = function ()
    local ok, err = mp.command(string.format("async no-osd vf add @vrrev:v360=sg:flat:in_stereo=2d:out_stereo=2d:id_fov=240.0:d_fov=%s:yaw=%s:pitch=%s:roll=%s:w=%s*135.0:h=%s*135.0",dfov,yaw,pitch,roll,res,res))
end
